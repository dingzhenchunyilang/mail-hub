<template>
  <div class="min-h-screen bg-paper">
    ...
    <button
      ...
      class="fixed top-4 left-4 z-50 p-2 border border-line-soft rounded-card bg-paper lg:hidden"
    >
      <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        <path v-if="!mobileMenuOpen" stroke-linecap="round" d="M4 6h16M4 12h16M4 18h16" />
    ...
    <div
      v-if="mobileMenuOpen"
      class="fixed inset-0 bg-ink/20 z-30 lg:hidden"
    ...
    <aside
      :class="[
        'fixed inset-y-0 left-0 w-60 bg-paper border-r border-line-soft z-40 transition-transform duration-200',
        mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      ]"
    >
      <div class="flex flex-col h-full">
        <!-- Logo -->
        <div class="flex items-center h-16 px-5 border-b border-line-soft">
          ...
          <span class="ml-3 text-lg font-serif font-semibold text-ink tracking-tight">Mail Hub</span>
        </div>

        <!-- Navigation -->
        <nav class="flex-1 px-3 py-5 space-y-0.5 overflow-y-auto">
          <!-- Mail section -->
          <p class="section-title px-3 mb-2">邮件</p>
          ...
          <router-link
            ...
            @click="mobileMenuOpen = false"
            :class="[
              'flex items-center px-3 py-2 rounded-card text-sm transition-colors duration-100',
              isActive(item.to)
                ? 'bg-ink text-paper'
                : 'text-ink hover:bg-paper-dim'
            ]"
          >
            <svg class="w-4 h-4 mr-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" :d="item.icon" />
            </svg>
            <span>{{ item.label }}</span>
            <span
              v-if="item.to === '/inbox' && stats.unread > 0"
              class="ml-auto text-[10px] font-mono text-stamp-red font-medium"
            >{{ stats.unread }}</span>
          </router-link>

          <!-- Divider -->
          <div class="my-4 border-t border-line-soft" />

          <!-- Categories section -->
          <p class="section-title px-3 mb-2">分类管理</p>
          ...
          <!-- System section -->
          <p class="section-title px-3 mb-2">系统</p>
        ...
        <div class="px-5 py-4 border-t border-line-soft">
          <div class="flex items-center justify-between mb-2">
            <div class="font-mono text-[11px] text-ink-faint space-y-1 flex-1">
              <div class="flex justify-between">
                <span>total</span>
                <span class="text-ink">{{ stats.total }}</span>
              </div>
              <div class="flex justify-between">
                <span>unread</span>
                <span class="text-stamp-red">{{ stats.unread }}</span>
              </div>
            </div>
...
onMounted(() => { if (dialogRef.value) setDialogRef(dialogRef.value); });
...
const mailNav = [
  { to: '/', label: '总览', icon: 'M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z' },
